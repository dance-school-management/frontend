...
export default async function Page() {
  ...
  const ongoingCourses = transformCourseAttendanceToProgressItems(
    courseAttendanceRate.data?.courseAttendancesRates || [],
  );
    ...
    <div className="flex h-full p-4 flex-col space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {instructorChartData.length > 0 && (
          <ProgressChart data={instructorChartData} title="Time spent with instructors" />
        )}
        <div>
          <Card>
            <CardHeader>
            ...
            <CardContent>
              <ProgressItemGroup
                data={ongoingCourses}
                emptyStateTitle={ongoingCoursesEmptyState.title}
                emptyStateDescription={ongoingCoursesEmptyState.description}
              />
            </CardContent>
          </Card>
        </div>
        <div>
          <Card>
            ...
            <CardContent>
              <ProgressItemGroup
                data={mastered}
                emptyStateTitle={masteredEmptyState.title}
                emptyStateDescription={masteredEmptyState.description}
              />
            </CardContent>
          </Card>
        </div>
